...
class TestData(Dataset):
    def __init__(self, audio_dir: Path, markup_path) -> None:
        ...
        test_audio_filepaths = sorted(list(audio_dir.glob("*.wav")))
        # classes = ["none"] * len(test_audio_filepaths)
        for file_name, category in zip(test_audio_filepaths, markup.values):
            audio, _ = torchaudio.load(file_name)
            self.audio_paths.append(audio)
            self.classes.append(category[-1])
