...
from dataset_prepare import prepare

noises_paths = (Path(__file__).parent / "../data/noises").iterdir()
device = "cuda"
...
class FineTune(sb.Brain):
    ...
    def prepare_features(self, wavs, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            # wavs = self.__add_noise(wavs, noise, 0, 6)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        _, lens = batch.sig
        class_id, _ = batch.class_id_encoded
        ...
        loss = sb.nnet.losses.nll_loss(predictions, class_id, lens)

        # Append this batch of losses to the loss metric for easy
        self.loss_metric.append(
            batch.id, predictions, class_id, lens, reduction="batch"
        )

        # Compute classification error at test time
        if stage != sb.Stage.TRAIN:
            self.error_metrics.append(batch.id, predictions, class_id, lens)
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.takes("class_id")
    @sb.utils.data_pipeline.provides("class_id", "class_id_encoded")
    def label_pipeline(class_id):
        yield class_id
        class_id_encoded = label_encoder.encode_label_torch(class_id)
        yield class_id_encoded
    ...
    for dataset in ["train", "valid"]:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            output_keys=["id", "sig", "class_id_encoded"],
        )
...
if __name__ == "__main__":
    ...
    hparams_file = Path(__file__).parent / "train.yaml"
    run_opts = {
        "debug": False,
        "debug_batches": 2,
        "debug_epochs": 2,
    ...
    class_id_brain.fit(
        epoch_counter=class_id_brain.hparams.epoch_counter,
    ...
    # test_stats = class_id_brain.evaluate(
